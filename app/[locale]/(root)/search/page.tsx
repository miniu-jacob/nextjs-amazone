// app/[locale]/(root)/search/page.tsx
    ...
    <div>
      ...
      <div className="mb-2 py-2 md:border-b flex-between flex-col md:flex-row">
        <div className="flex items-center text-sm">
          ...
          {data.totalProducts === 0 ? t("Search.No") : `${data.from}-${data.to} ${t("Search.of")} ${data.totalProducts}`}{" "}
          {t("Search.results")}
          ...
          {/* 개별 필터 값 표시 */}
          {q !== "all" && q !== "" && '"' + q + '"'}
          {category !== "all" && category !== "" && ` ${t("Search.Category")}: ` + category}
      ...
      <div className="bg-card grid md:grid-cols-5 md:gap-4">
        ...
        <CollapsibleOnMobile title={t("Search.Filters")}>
          <div className="space-y-4">
            <div>
              <div className="font-semibold text-base">{t("Search.Department")}</div>
            ...
            {/* 별점 필터 (리뷰) */}
            <div>
              <div className="font-bold text-base">{t("Search.Customer Review")}</div>
              <ul className="text-sm space-y-1">
                <li>
                ...
                <li>
                  <Link href={getFilterUrl({ rating: "4", params })} className={`${"4" === rating && "text-primary"}`}>
                    <div className="flex items-center">
                      ...
                      <span className="px-2 text-sm">{t("Search.& Up")}</span>
                    </div>
                  </Link>
                </li>
              </ul>
        ...
        <div className="md:col-span-4 space-y-4 ">
          <div className="mt-2">
            ...
            <div className="text-sm mt-2">{t("Search.Check each product page for other buying options")}</div>
          </div>
          ...
          {/* 페이지네이션 */}
          <div className="w-full flex justify-center">{data.totalPages > 1 && <Pagination page={page} totalPages={data.totalPages} />}</div>
        </div>
      </div>
    </div>
  );
