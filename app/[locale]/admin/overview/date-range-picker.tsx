...
export function CalendarDateRangePicker({
  ...
  const dateFnsLocale = locale === "ko-KR" ? ko : locale === "en-US" ? enUS : vi;

  const fromDate = calendarDate?.from ? formatDateTime(calendarDate.from, locale).dateOnly : t("DatePicker.Pick a date");
  const toDate = calendarDate?.to ? formatDateTime(calendarDate.to, locale).dateOnly : "";
        ...
        <PopoverTrigger asChild>
          <Button
            ...
            {calendarDate?.from ? (
              calendarDate.to ? (
                <>
                  {t("DatePicker.From", { from: fromDate })} - {t("DatePicker.To", { to: toDate })}
                </>
              ) : (
                // 날짜의 처음만 있을 때는 처음 날짜만 표시
                t("DatePicker.From", { from: fromDate })
              )
            ) : (
              // 날짜가 없을 때는 아래 텍스트 표시
              <span>{t("DatePicker.Pick a date")}</span>
            )}
          </Button>
        </PopoverTrigger>
        <PopoverContent
          ...
          <div className="flex gap-4 p-4 pt-0">
            <PopoverClose asChild>
              <Button onClick={() => setDate(calendarDate)}>{t("DatePicker.Apply")}</Button>
            </PopoverClose>
            <PopoverClose asChild>
              <Button variant={"outline"}>{t("DatePicker.Cancel")}</Button>
            </PopoverClose>
          </div>
        </PopoverContent>
      </Popover>
